...
- **Visual:** dark body (`#1a1a1a`), green LED digits (`#66ff66`), specular `0.05`, `edgeR: 0.10` (slightly sharper), `notchD: 0` (flat faces, no pip depressions). **All faces use 7-segment display digits instead of pips** (DynamicTexture per face, procedural canvas paths): faces 1, 2, 4, 5, 6 show their number as a single 7-segment digit. **Face 3** shows three 7-segment digits — `3`, `1`, `4` — placed in the standard 3-pip diagonal positions (π easter egg). The die looks like a digital calculator/LED display — unique in the roster.
