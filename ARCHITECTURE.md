...
| **UI overlay layout** | `.battle-scene` fixed over the viewport (`z-index` above canvas). CSS variable **`--battle-ui-top`** sets `top` for side columns. **Side panels** (`.side-left` / `.side-right`): compact HP widget (avatar circle 80px + bar with inline HP numbers), 3 dashed badge placeholder slots (`.badge-grid`), side turn indicators (`.side-turn` — text-only, no bg, fade via `.active`). **Center** (`.center`): `.info-bar` docked to top edge (round score + selection info; turn indicator removed). `.actions` at bottom (primary row: ROLL / SCORE / BANK; secondary row reserved for ability buttons). Mid-screen `pointer-events:none` — clicks pass through to 3D canvas. **Banner**: centered on viewport (fade+scale, 2200ms timeout, ~3.4s before bot turn starts). Round History in `.side-right` under bot block. |
| **Physics defaults** | **`BATTLE_TUNE_DEFAULTS`** in `battle.html` seeds `battleTune` before **`localStorage['battle_tune_json_v1']`** is merged (numeric fields only). Hash-based versioning auto-clears stale `localStorage` when code defaults change. **Tune → Apply** saves the full tune; **Reset** removes the key. **`throw-lab.mjs`** duplicates the same defaults object — **keep both files in sync** when changing shipped feel. **Baseline v2 (current):** gravity −200, mass 3.0, throwMin/Max 66/166, dieScale 3.06, solver.iterations 20, invisible ceiling at FLOOR_Y+18, specularColor 0.08, directional light Y=50 |
| **Table layout** | Sandwich along Z: **roll zone** (center, `ROLL_DEPTH_Z`, stretch `DEFAULT_ROLL_DEPTH_STRETCH=2.8`) + **held zones** (player / bot). Held zones are **9 x 6 unit rectangles** (`SHELF_W=9`, `SHELF_D=6`) centered on each side; inner edge flush with the physics divider, zone expands outward. Thin **border line** (`CreateLines`, warm brown, alpha 0.7) outlines the roll zone perimeter. Physics **walls** at `DIVIDER_Z` / `BOT_DIVIDER_Z` confine dice to the rolling band. Ortho frustum padding: `uiPadH=12`, `uiPadV=5`. Constants: `TABLE_WORLD_SCALE`, `ROLL_FLOOR_W`, `SHELF_W`, `SHELF_D`, `ROLL_DEPTH_Z`, `DIVIDER_Z`, `BOT_DIVIDER_Z`, `WX`, etc. |
