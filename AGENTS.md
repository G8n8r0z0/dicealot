...
### 1. HTTP vs `file:///` (CRITICAL)

**Target game shell (`src/`, store, systems):**
- Game logic MUST avoid ES modules — load via `<script>` in order, IIFE + `window` exports, so **`src/index.html` can work from `file://`** where practical.
- No bundler at runtime for that layer.

**3D prototypes (`battle.html`, `spike-v2.html`):**
- Use `<script type="module">` + import map → **`file://` is unsupported** (browser CORS / module rules). Players must use **`node server.mjs`**, GitHub Pages, or another HTTP host.
- Dependencies are **vendored** under `vendor/` (no CDN at runtime). `npm install` + `npm run vendor:sync` is for refreshing those files only.

**Root `index.html`:** on `file://`, shows a short message (RU) with server instructions instead of redirecting to `battle.html`.
