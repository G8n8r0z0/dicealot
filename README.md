...
**Battle prototype (`battle.html`)** today: fullscreen 3D Farkle vs bot, **vendored** Babylon.js + cannon-es, **pull-back sling** + **ROLL**, **camera debug** (orbit + copy JSON). **UI baseline (2026-04-07):** compact HP widgets with avatar circles, side turn indicators, top-docked info bar (round + selection score), centered banners, 9x6 held zones, badge placeholder slots, two-row action buttons. **`throw-lab.html`** — throw-only sandbox with the same `battle_tune_json_v1` tuning. Details: `ARCHITECTURE.md` (Battle Prototype, Throw lab), `DESIGN.md` section 10.6 + 14.
